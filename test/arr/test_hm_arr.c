...
void test_arr_fixed_shrink_to_fit() {
    int fail_cnt = 0;
    int tag = 0;
    print_run("ARR(FIXED) | FUNC | SHRINK TO FIT| CAPACITY: 64");
    ...
    int fail = 0;
    for (int i = 0; i < capacity; i++) {
        if (hm_arr_shrink_to_fit(&arr) != hm_arr_ret_none) {
            fail++;
        }
        test_arr_integrity(&arr, &fail_cnt, tag++, capacity - i, false, capacity, free);
        hm_arr_del_tail(&arr);
    ...
    print_end("ARR(FIXED) | FUNC | SHRINK TO FIT| CAPACITY: 64", fail_cnt);
    HM_TEST_COUNTER
}

void test_arr_dynamic_shrink_to_fit() {
    int fail_cnt = 0;
    int tag = 0;
    print_run("ARR(DYNAMIC) | FUNC | SHRINK TO FIT| CAPACITY: 64");
    ...
    int fail = 0;
    for (int i = 0; i < start_capacity * 2; i++) {
        if (hm_arr_shrink_to_fit(&arr) != hm_arr_ret_suc) {
            fail++;
        }
        check_res(arr.capacity == start_capacity * 2 - i, "the capacity should be equal to size when shrink to fit on a dynamic-grow arr", &fail_cnt, tag++);
        test_arr_integrity(&arr, &fail_cnt, tag++, start_capacity * 2 - i, true, start_capacity * 2 - i, free);
        hm_arr_del_tail(&arr);
    }
    check_res(fail == 0, "shrink function should return suc when shrink to fit on a dynamic-grow arr", &fail_cnt, tag++);

    hm_arr_free(&arr);

    print_end("ARR(DYNAMIC) | FUNC | SHRINK TO FIT| CAPACITY: 64", fail_cnt);
    HM_TEST_COUNTER   
}
...
void test_empty_fixed_arr_oper() {
    int fail_cnt = 0;
    int tag = 0;
    print_run("ARR(FIXED) | BOUNDARY | OPER EMPTY ARR | CAPACITY: 64");
...
void test_empty_dynamic_arr_oper() {
    int fail_cnt = 0;
    int tag = 0;
    print_run("ARR(DYNAMIC) | BOUNDARY | OPER EMPTY ARR | CAPACITY: 64");
    ...
    /* shrink */
    hm_arr_init_dynamic_grow(&arr, capacity, NULL);
    check_res(hm_arr_shrink(&arr) == hm_arr_ret_suc, "shrink on empty and dynamic-grow arr should return suc", &fail_cnt, tag++);
    test_arr_integrity(&arr, &fail_cnt, tag++, 0, true, capacity, NULL);
    hm_arr_free(&arr);
...
void test_full_fixed_arr_oper() {
    int fail_cnt = 0;
    int tag = 0;
    print_run("ARR(FIXED) | BOUNDARY | OPER FULL ARR | CAPACITY: 64");
    ...
    *val = -1;
    ...
    check_res(cnt == 0, "the val shouldn't existed in arr when insert this val in a full and fixed-size arr", &fail_cnt, tag++);
    hm_arr_free(&arr);
    free(val);


    /**
     *  insert a val at tail
     * 
     */
    hm_arr_init(&arr, capacity, free);
    /* insert to full */
    ...
    *val = -1;
    ...
    print_end("ARR(FIXED) | BOUNDARY | OPER FULL ARR | CAPACITY: 64", fail_cnt);
    HM_TEST_COUNTER
}

void test_full_dynamic_arr_oper() {
    int fail_cnt = 0;
    int tag = 0;
    print_run("ARR(DYNAMIC) | BOUNDARY | OPER FULL ARR | CAPACITY: 64");
    ...
    *val = -1;
    ...
    check_res(cnt == 1, "the val shouldn existed in arr when insert this val in a full and dynamic arr", &fail_cnt, tag++);
    hm_arr_free(&arr);
    ...
    *val = -1;
    ...
    check_res(cnt == 1, "the val shouldn existed in arr when insert this val in a full and dynamic-grow arr", &fail_cnt, tag++);
    hm_arr_free(&arr);
    ...
    *val = -1;
    ...
    check_res(cnt == 1, "the val should existed in arr when insert this val in a full and dynamic-grow arr", &fail_cnt, tag++);
    hm_arr_free(&arr);

    
    print_end("ARR(DYNAMIC) | BOUNDARY | OPER FULL ARR | CAPACITY: 64", fail_cnt);
    HM_TEST_COUNTER
}


void test_no_capacity_fixed_arr_oper() {
    int fail_cnt = 0;
    int tag = 0;
    print_run("ARR(FIXED) | BOUNDARY | OPER NO CAPACITY ARR | CAPACITY: 64");
    ...
    test_arr_integrity(&arr, &fail_cnt, tag++, 0, false, capacity, NULL);
    check_res(hm_arr_get(&arr, 100) == NULL, "get with a large index on 0-capacity arr should return NULL", &fail_cnt, tag++);
    test_arr_integrity(&arr, &fail_cnt, tag++, 0, false, capacity, NULL);
    hm_arr_free(&arr);
    ...
    test_arr_integrity(&arr, &fail_cnt, tag++, 0, false, capacity, NULL);
    check_res(hm_arr_get_pointer(&arr, 100) == NULL, "get_pointer with a large index on 0-capacity arr should return NULL", &fail_cnt, tag++);
    test_arr_integrity(&arr, &fail_cnt, tag++, 0, false, capacity, NULL);
    hm_arr_free(&arr);
    ...
    test_arr_integrity(&arr, &fail_cnt, tag++, 0, false, capacity, NULL);
    check_res(hm_arr_pop(&arr, 100) == NULL, "pop with a large index on 0-capacity arr should return NULL", &fail_cnt, tag++);
    test_arr_integrity(&arr, &fail_cnt, tag++, 0, false, capacity, NULL);
    hm_arr_free(&arr);
    ...
    print_end("ARR(FIXED) | BOUNDARY | OPER NO CAPACITY ARR | CAPACITY: 64", fail_cnt);
    HM_TEST_COUNTER
}

void test_no_capacity_dynamic_arr_oper() {
    int fail_cnt = 0;
    int tag = 0;
    print_run("ARR(DYNAMIC) | BOUNDARY | OPER NO CAPACITY ARR | CAPACITY: 64");
    ...
    test_arr_integrity(&arr, &fail_cnt, tag++, 1, true, capacity, NULL);
    check_res(hm_arr_get(&arr, 0) == &v, "val is wrong when inset head in 0-capacity and dynamic-grow arr", &fail_cnt, tag++);
    hm_arr_free(&arr);
    ...
    print_end("ARR(DYNAMIC) | BOUNDARY | OPER NO CAPACITY ARR | CAPACITY: 64", fail_cnt);
    HM_TEST_COUNTER
}


void test_init_big_capacity_fixed_arr() {
    int fail_cnt = 0;
    int tag = 0;
    print_run("ARR(FIXED) | BOUNDARY | INIT BIG CAPACITY ARR");
    ...
    for (size_t i = 1; i <= cnt; i++) {
        ...
        sprintf(info, "init %zu-capacity arr should return error", capacity);
        ret = hm_arr_init(&arr, capacity, free);
        check_res(ret == hm_arr_ret_error, info, &fail_cnt, tag++);
    }
    

    print_end("ARR(FIXED) | BOUNDARY | INIT BIG CAPACITY ARR", fail_cnt);
    HM_TEST_COUNTER
}


void test_init_big_capacity_dynamic_arr() {
    int fail_cnt = 0;
    int tag = 0;
    print_run("ARR(DYNAMIC) | BOUNDARY | INIT BIG CAPACITY ARR");
    ...
    for (size_t i = 1; i <= cnt; i++) {
        size_t capacity = SIZE_MAX / i;
        sprintf(info, "init %zu-capacity arr should return error", capacity);
        ret = hm_arr_init_dynamic_grow(&arr, capacity, free);
        check_res(ret == hm_arr_ret_error, info, &fail_cnt, tag++);
    }
    
    
    print_end("ARR(DYNAMIC) | BOUNDARY | INIT BIG CAPACITY ARR", fail_cnt);
    HM_TEST_COUNTER
...
void test_arr_fixed_boundary() {
    test_empty_fixed_arr_oper();                                                        printf("\n");

    test_full_fixed_arr_oper();                                                         printf("\n");

    test_no_capacity_fixed_arr_oper();                                                  printf("\n");

    test_init_big_capacity_fixed_arr();                                                 printf("\n");
}


void test_arr_dynamic_boundary() {
    test_empty_dynamic_arr_oper();                                                      printf("\n");

    test_full_dynamic_arr_oper();                                                       printf("\n");

    test_no_capacity_dynamic_arr_oper();                                                printf("\n");

    test_init_big_capacity_dynamic_arr();                                               printf("\n");
}
...
void boundary_test() {
    test_arr_fixed_boundary(); 

    test_arr_dynamic_boundary();
}
