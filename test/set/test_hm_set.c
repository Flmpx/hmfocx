...
void test_set_integrity(hm_set* set, int* fail_cnt, int tag, size_t size, hm_hash hash_key, hm_cmp cmp_key, hm_free free_key) {
    ...
    check_res(set->cmp_key == cmp_key, "TEST OF INTEGRITY: set's cmp_key is unexpected", fail_cnt, tag);
    check_res(set->hash_key == hash_key, "TEST OF INTEGRITY: set's hash_key is unexpected", fail_cnt, tag);
    check_res(set->free_key == free_key, "TEST OF INTEGRITY: set's free_key is unexpected", fail_cnt, tag);

    check_res(!(set->len == 0 && set->buckets != NULL), "TEST OF INTEGRITY: set's len is 0, but buckets isn't NULL", fail_cnt, tag);
    check_res(!(set->len == 0 && set->buckets_status != NULL), "TEST OF INTEGRITY: set's len is 0, but buckets_status isn't NULL", fail_cnt, tag);
    ...
    check_res(set->size == size, "TEST OF INTEGRITY: set's size isn't the expected size", fail_cnt, tag);
    check_res(set->size <= set->len, "TEST OF INTEGRITY: set's `len` isn't greater than `size`", fail_cnt, tag);
    check_res(existed == set->size, "TEST OF INTEGRITY: the real size is different from set's size", fail_cnt, tag);
...
void test_set_init() {
    ...
    test_set_integrity(&set_1, &fail_cnt, tag++, 0, hash_int_1, cmp_int_up, free);
    ...
    test_set_integrity(&set_2, &fail_cnt, tag++, 0, hash_int_1, cmp_int_up, NULL);
...
void test_set_shrink() {
    ...
    test_set_integrity(&set, &fail_cnt, tag++, num / 2, hash_int_1, cmp_int_up, free);
...
void test_set_insert_stress() {
    ...
    for (int i = 0; i < cnt; i++) {
        ...
        test_set_integrity(&set, &fail_cnt, tag++, suc, hash_int_1, cmp_int_up, NULL);
...
void test_set_get_stress() {
    ...
    for (int i = 0; i < cnt; i++) {
        ...
        clock_t end = clock();
        test_set_integrity(&set, &fail_cnt, tag++, nums[i], hash_int_1, cmp_int_up, free);
...
void test_set_pop() {
    ...
    for (int i = 0; i < cnt; i++) {
        ...
        test_set_integrity(&set, &fail_cnt, tag++, num - pop_cnt, hash_int_1, cmp_int_up, free);
    }
