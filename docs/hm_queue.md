...
>  [!Note]  
>  - Most of the functions will **abort**(in debug build) when `hm_queue*` is `NULL`, so, you have to make sure the pointer is valid
>  - **Exception:** Pass `NULL` in `hm_queue_free()` is allowed
