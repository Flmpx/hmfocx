...
>  [!Note]  
>  - Most of the functions will **abort**(in debug build) when `hm_map* & hm_map_iter*` is `NULL`, so, you have to make sure the pointer is valid
>  - **Exception:** Pass `NULL` in `hm_map_free()` is allowed
